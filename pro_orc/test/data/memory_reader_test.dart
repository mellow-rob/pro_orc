...
void main() {
  group('MemoryReader', () {
    ...
    group('readMemoryData', () {
      ...
      test(
        'returns hasMemory=false when project dir exists but no MEMORY.md',
        () async {
          final claudeHome = await Directory.systemTemp.createTemp(
            'claude_home_',
          );
      ...
      test(
        'returns isStale=true when MEMORY.md is older than 7 days',
        () async {
          final claudeHome = await Directory.systemTemp.createTemp(
            'claude_home_',
          );
      ...
      test(
        'finds memory for a project with a dot in its name (fuzzy match)',
        () async {
          final claudeHome = await Directory.systemTemp.createTemp(
            'claude_home_',
          );
