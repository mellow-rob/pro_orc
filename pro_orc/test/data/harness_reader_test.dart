...
void main() {
  group('HarnessReader', () {
    test('reads hooks, permissions, env and inline MCP across three levels',
        () async {
...
''');
      await File(p.join(projClaude.path, 'settings.local.json'))
      ...
      final allowGlobal = data.permissions
          .where((x) => x.level == HarnessLevel.global && x.kind == 'allow');
      expect(allowGlobal.map((x) => x.rule), containsAll(['Read', 'Bash(git *)']));
      expect(
        data.permissions.any((x) =>
      ...
      expect(data.envVars.map((e) => '${e.key}=${e.value}'),
    ...
    test('reads project .mcp.json in both shapes', () async {
      ...
      final srv =
          data.mcpServers.firstWhere((m) => m.name == 'proj-srv');
    ...
    test('extracts H1 titles from rules, falling back to filename', () async {
      ...
      await File(p.join(rulesDir.path, 'testing.md'))
          .writeAsString('# Testing Requirements\n\nsome body');
      await File(p.join(rulesDir.path, 'no-heading.md'))
      ...
      final withH1 =
          data.rules.firstWhere((r) => r.relativePath.endsWith('testing.md'));
      expect(withH1.title, 'Testing Requirements');
      final noH1 = data.rules
          .firstWhere((r) => r.relativePath.endsWith('no-heading.md'));
      expect(noH1.title, 'no-heading.md');
    });

    test('malformed JSON and missing files yield empty results, never throws',
        () async {
      final claudeHome = await Directory.systemTemp.createTemp('harness_home_');
      addTearDown(() => claudeHome.delete(recursive: true));

      // Broken global settings.
      await File(p.join(claudeHome.path, 'settings.json'))
