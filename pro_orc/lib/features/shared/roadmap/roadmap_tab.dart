...
///
/// The tier-0 milestone selection ([selectedMilestone]/[onMilestoneSelected])
/// is controlled from [ProjectDetailPanel] (feature 002, Wave 1) rather than
/// owned here — Zeitstrahl is now a sibling top-level tab instead of an
/// in-tab view toggle, so the selection has to survive a round-trip through
/// that sibling tab, which means it must live above both.
class RoadmapTab extends ConsumerWidget {
  ...
  /// Currently selected tier-0 milestone (lanes drill-down), or null. Unused
  /// for non-tier-0 sources (the legacy tree/spec-list path keeps its own
  /// internal phase/spec selection).
      ...
      data: (result) {
        ...
        if (result.source == RoadmapSource.productStore) {
          return _RoadmapHeroView(
            ...
            onMilestoneSelected: onMilestoneSelected ?? (_) {},
          );
        }
...
/// Tier-0 (docs/product/) Roadmap view: hero ("Wo stehen wir" from NEXT.md)
/// + milestone lanes with click-to-drill-down feature cards.
///
/// Stateless — the milestone selection is fully controlled by the caller
/// ([RoadmapTab.selectedMilestone]/[RoadmapTab.onMilestoneSelected]) so it
/// can be hoisted above the Roadmap/Zeitstrahl tab split in
/// `ProjectDetailPanel` (feature 002, Wave 1) and survive switching to the
/// Zeitstrahl tab and back.
