...
## Current State (main @ f2931a5 — 2026-07-22)

v1.0–v3.3.1 shipped/released. Since v3.3.1: Complete Project Deletion's Vercel wave finished (active Vercel project deletion via `vercel` CLI) plus two resource-detection bugfixes (allowlist instead of blacklist for `.md`-scanned URLs; reads `.vercel/project.json` directly so real Vercel projects without a linked doc-URL are still detected). Then two new features shipped: Pre-Flight Permission Check (GitHub `delete_repo` scope is checked when the checkbox is ticked, not only at execution time, with an actionable missing-permission popup) and a repo-owner hint in that popup, followed by a bugfix for `gh auth refresh`'s single-active-account limitation (distinguishes an account mismatch and falls back to `gh auth login` with a switch-back hint). No new version tag cut yet — installed locally at build 3.3.1+16 binary but built from newer source; a formal release is pending.

### Build Baseline
- `flutter test`: 740 tests, 0 failures
- `flutter analyze`: 0 issues
- Distribution: `scripts/build-dmg.sh` builds DMG installer, GitHub Actions release on tag push; for quick local iteration, `flutter build macos --release` + `codesign --deep --force -s -` + copy to `/Applications` skips the full release/Homebrew cycle (agreed convention: only tag a formal release for bundled milestones, not every fix)
